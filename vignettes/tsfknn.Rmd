...
rolling origin evaluation. The object returned by `rolling_origin` contains the results. For
...
Every row of the matrix contains a different test set. The first row is a test set with the last `h` values of the time series, the next row a test set with te last `h` -  1 values of the time series and so on. Each test set has its associated training test with all the data in the time series not belonging to the test set. For every training data a KNN model with the parameters associated to the original model is built and the test set is predicted. You can see the predictions as follows:
...
The different forecast accuracy measures applied to all the errors can be consulted:

```{r}
ro$global_accu
```

It is also possible to consult forecast accuracy measures for every forecast horizon:
